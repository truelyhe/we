const envid = 'pikaqiu-5sfst'; //云开发环境id
// 云函数入口文件
    ...
    try {
      await db.collection('addpeople').where({
       ...
       await db.collection('user').where({
        ...
        data:{
          friends: db.command.push([{ id: event.peopleconfim.chatid, userInfo: event.peopleconfim.peopleadd }])
        }
      })
      await db.collection('user').where({
        ...
        data: {
          friends: db.command.push([{ id: event.peopleconfim.chatid, userInfo: event.peopleconfim.peopleask }])
        }
      })
